// Simple state center coordinates for basic mapping
...
const STATE_COORDINATES: Record<string, { lat: number; lng: number }> = {
	...
	Montana: { lat: 47.077978, lng: -110.362566 },
	...
	'West Virginia': { lat: 38.491226, lng: -80.95457 },
	Wisconsin: { lat: 44.268543, lng: -89.616508 },
	Wyoming: { lat: 42.755966, lng: -107.30249 }
};

export function addCoordinatesToResource(resource: any): any {
	const state = resource.address?.state;
...
export function addCoordinatesToResources(resources: any[]): any[] {
	return resources.map(addCoordinatesToResource);
}
